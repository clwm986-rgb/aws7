...
public class Ex09_동기화 {
	...
	public static void main(String[] args) {
       ...
       * */
		...
		//홍길동이 본인 계좌에 10000이 있
		
		//홍길동이 본인 계좌에 10000을 입금
		Customer customer1 = new Customer("홍길동", account);
		customer1.start();
		//홍길동 아빠가 아들 계좌에 10000 입금
		 ...
		 * 
		 * 동기화 후 결과 (synchronized 사용 후)
		 * -> 홍길동 입금 처리가 완료되기 전에 아빠가 입금을 시도하지만 동기화 때문에 입금 시도를 막고 잇다가,
		 *  홍길동 입금 처리가 완료 되면 아빠 입금 처리가 진행도서 정상 결과가 나옴
		 * 입금 전 금액 : 10000
...
class BankAccount{
	...
	public synchronized void deposit(String name, int money) {
		...
		if(money <= 0) {
			System.out.println("입금 액은 0보다 커야합니다.");
			return;
		}
		balance += money;
		
		try {
			Thread.sleep(2000);// 입금 중, 2초동안 스레드를 멈품
		} catch (Exception e) {
