...
public class Ex10_컬렉션프레임워크 {
	public static void main(String[] args) {
		/* 컬렉션 프레임워크 p.422
	       ...
	       *   - Map 인터페이스
	       *   
	       * Collection 인터페이스  (List와 Set에 공통으로 잇는 기능)
	       * - 기능 
	       *   - add(값) : 값을 뒤에 추가 
		...
		System.out.println("임꺽정이 잇습니까?" + list.contains("임꺽정"));
		System.out.println("임꺽정이 몇번지에 잇습니까?" + list.indexOf("임꺽정"));
		System.out.println("1번지에 누가 잇습니까?" + list.get(1));
		
		list.set(0, "둘리"); // 0번지에 잇는 임꺽정을 둘리로 수정
		System.out.println(list);
		
		list.add("하니");
		System.out.println(list);
		
		list.remove(0);//0번지에 잇는 이름을 제거
