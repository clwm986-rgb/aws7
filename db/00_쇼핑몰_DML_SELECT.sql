...
JOIN category ON ca_code = category.code
WHERE category.name = '악세서리';
    
# abc123회원이 장바구니에 담은 제품의 제품명과 수량을 조회 
SELECT * FROM cart
JOIN product ON product.code = cart.code 		# product using(code)
WHERE id = 'abc123';
    
# 검색어를 목걸이로 검색했을 때 결과를 조회 => like, % 
...
# ACC002, 18K반지, 200000, 1 
select code as 제품코드, name as 제품명, sum(buy.amount) as 판매량, sum(product.price * buy.amount) as 판매액
from buy join product using (code)
group by code;

# 회원별 제품구매횟수
SELECT id AS 회원아이디, count(*) AS 횟수
FROM buy
GROUP BY id;

        
# 회원 아이디, 제품구매횟수
SELECT id AS 회원아이디, COUNT AS 구매횟수
FROM buy
GROUP BY id;
