# 내장함수
# if(수식, 수식1, 수식2) : 수식이 참이면 수식1을, 거짓이명 수식2를 리턴
set @age = 10;
select if(@age > 19, '성인', '미성년자') as '나이는';

# ifnull(수식1, 수식2) : 수식1이 null이 아니면 수식1을, null이면 수식2를 반환
	# null값일 때 넣을 기본값 설정할 떄 사용
    
set @age2 = null;
select ifnull(@age2,0), ifnull(@age2,0);

#nullif(수식1, 수식2) : 수식1과 수식2과 같으면 null, 다르면 수식1을 반환
set @age3 = 10;
select nullif( @age, @age3), ifnull( @age, @age2);
...
# else 결과
# end
select case when @age > 19 then '성인'
	else '미성년자'
    end as 결과;
