...
2. outer join
 - 기준 테이블을 기준으로 합침
	# 기준 테이블 옆에 다른 테이블을 가져와서 값이 잇는 애들은 붙이고 없으면 null을 채움
 - 재품 별 판매액을 조회할 떄 사용
	# 판매 안된 제품은 0월으로 조회할 때 사용
 - left join / right join
	# 테이블 A left join 테이블B -> 테이블 A를 기준으로 테이블B를 연결
    # 테이블 A right join 테이블B -> 테이블 B를 기준으로 테이블A를 연결
*/
# 모든 제품의 판매 기록을 조회 : OUTER JOIN
# 판매된 제품들만 판매 기록을 조회 : INNER JOIN
USE SAMPLE2;
#모든 학생의 학생별 들은 과목수
select NAME 이름, COUNT(*) AS 과목수
FROM SCORE RIGHT JOIN STUDENT ON ST_CODE = STUDENT.CODE
...
# 서브쿼리
	# 뭐리안에 들어가는 쿼리
    # SELECT문 안에 들어가는 SELECT
# 장점
	# 쿼리를 구조화 시킴
    # JOIN보다 가독서잉 좋음
    # 성능을 향상시킬 수 잇음
# 서브쿼리 사용 위치 
	# SELECT문에서 컬럼 대신 사용 가능
    # FROM에서 테이블 대신 사용 가능 : 이때 AS 테이블 별칭
    # WHERE절에서 특정 값 대신 사용
    # HAVING 절에서 특정 ㄱ밧 대신 사용
    # INSERT문에서 VALUES 대신 사용
    # UPDATA문에서 값으 검색해서 꿀 떄 사용